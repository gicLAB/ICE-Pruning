...
def get_model():
    if args.model_name == 'mobile':
        model = MobileNetV2()
        args.checkpoint = 'mobile/models/mobilev2.th'
...
def freeze_model(model, original_model, train_loader, val_loader, fr):
    ...
    layers = model.module.layers
    for layer_index in range(len(layers)):
        freeze_layer(config, layers[layer_index], layer_index)
    return model, config
    
def freeze_layer(config, layer, l_index):
    name1 = 'module.layer' + str(l_index + 1) + '.' + 'conv1.weight'
    name2 = 'module.layer' + str(l_index + 1) + '.' + 'conv2.weight'
    if name1 in config:
...
def prune(freeze_config, model, original_model, train_loader, val_loader, depth, thre, base, p, beta, delta, auto_search=False):
    ...
    for layer_index in range(modules):
        ...
        layers[layer_index] = P.prune(model, train_loader, layers[layer_index], args.pruning_ratio, args.pruning_method)

        freeze_layer(freeze_config, layers[layer_index], layer_index)
        model = model.cuda()
...
parser.add_argument('--model_name', type=str, default='mobile')
...
weight_norm = get_weight_norm_mobile()
...
with open('/project/experiment/' + args.pruning_method + '_seed_' + str(args.seed) + '_ratio_' + str(args.pruning_ratio * 100) + '.pickle', 'wb') as handle:
