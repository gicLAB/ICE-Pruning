...
# ====================================================================================================================================
# num_pruningRound 90 0.6 target_loss 0.09
# lamb 0.2 works
for p in 0.6 0.7 0.8
do
	for s in 0
	do
		CUDA_VISIBLE_DEVICES=0,1 python3 non_change_dense/aap/main.py --data './cifar10' --num-classes 10 \
		--raport-file raport.json -j8 -p 100 \
		--workspace ${1:-./} --arch densnenet -c fanin \
		--label-smoothing 0.0 --lr-schedule step --lr_decay_epochs '91,136' --warmup 0 --mom 0.9 --wd 0.0002 \
		--lr 0.001 --optimizer-batch-size 512 --batch-size 256 \
		--prune_type AAP_givenAccLossParaReduce_minimizePara \
		--epochs 182 --rewind_epoch 0 --target_parameters_reduce $p --lambda_value 0.1 \
